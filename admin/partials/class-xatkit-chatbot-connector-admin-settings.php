...
class Xatkit_Chatbot_Connector_Admin_Settings {
     ...
     * @param Xatkit_Chatbot_Connector_Admin $admin Link with the main admin object.
     */
    ...
    public function init_settings() {
        ...
        add_settings_field(
            'urlXatkit',
            __('Full URL of the deployed Xatkit bot Server (including port if needed)','xatkitcom'),
        ...
        add_settings_field(
            'apikey',
            __('API Key to send to the bot server (if needed)','xatkitcom'),
        ...
        add_settings_field(
            'inAdmin',
            __('Display the widget in the admin pages?','xatkitcom'),
            array( $this, 'render_visibility_inadmin_field' ),
            'xatkit-visibility',
            'visibility'
        );
    ...
    public function render_apikey_field() {
	    ...
	    echo '<p>' . __('Leave blank if your xatkit server does not require an API key to start the communication').'</p>';
    }
    ...
    public function render_visibility_inadmin_field() {
        // Retrieve the full set of options
        $options = get_option( 'settingsXatkitWooVisibility' );
        echo '<div class="visibilityItem inadmin">';
        $checked = isset( $options['visibility']['inAdmin'] ) ? $options['visibility']['inAdmin'] : '0';
        $checked = esc_attr($checked);
        echo '<input type="checkbox" id="inadmin" name="settingsXatkitWooVisibility[visibility][inAdmin]" value="1"' . checked( 1, $checked, false ) . '/>';
        echo '</div>';
    }
